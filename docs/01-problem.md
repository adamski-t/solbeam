...
A BSV block arrives roughly every ten minutes. That is not the bottleneck. The bottleneck is **confirmation policy**: because a chain can reorganise, every custodian — exchanges especially — chooses how deep a deposit must be buried before they treat it as final, and how long a withdrawal must sit before they broadcast it. For BSV, those windows are long.
...
- **New users** — particularly in places where obtaining BSV is genuinely difficult — face a slow, gated on-ramp.
...
SOLBEAM takes a different starting point: **make the hard half trustless, and be honest about the other half.**
...
That asymmetry — trustless in, trust-minimised out — is the honest shape of the problem, and it is what this documentation sets out.
