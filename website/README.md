...
└── litepaper/
    └── index.html          ← placeholder; swap for the real litepaper
```
...
| Litepaper link target | `index.html` — the `.btn` href, and `.footer` link |
...
- **The docs site:** build the GitBook and either drop it in `litepaper/`, or host it on `docs.solbeam.me` and change the two links in `index.html`.
...
- [ ] Litepaper link points somewhere real when the litepaper exists
